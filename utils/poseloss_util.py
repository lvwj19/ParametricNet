...
def get_trans_loss(pred_trans, trans_label, weight=None):
    ...
    # l2 loss
    x = tf.sqrt(tf.reduce_sum(tf.square(trans_label - pred_trans), axis=-1)) # shape: n
    if weight is not None:
        x = tf.multiply(x, tf.reshape(weight, [-1]))
    return tf.reduce_mean(x)


def get_cos_distance(x3, x4):   
    ...
    cosin = tf.divide(x3_x4, tf.multiply(x3_norm, x4_norm))
    print('x3_norm',x3_norm.shape,x3_x4.shape)
    return tf.reduce_mean(tf.abs(cosin))
